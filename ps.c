...
int main()
{
    ...
    for (i = 0; i < amofproc; i++) {
        ...
        //here is an example of how to print stuff using write() syscall
        char* name = "user: ";
        char* name_with_var;
		name_with_var = malloc(strlen(name)+1+4); /* make space for the new string (should check the return value ...) */
		strcpy(name_with_var, name); /* copy name into the new var */
		strcat(name_with_var, s->user); /* add the extension */
		//write(fd[1], sizeof(name_with_var), sizeof(name_with_var));
        write(fd[1], name_with_var, 15);
        //write(1, ", ", 2);
        //syscall example ended
